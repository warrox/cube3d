...
/*   Updated: 2024/10/05 10:21:11 by whamdi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	return (map_x < 0 || map_x >= data->file->max_len || map_y < 0
		|| map_y >= data->file->line_map);
}

void	check_ray_hit(t_data *data, int *hit, int *map_x, int *map_y)
{
	*map_x = (int)data->ray_x;
	*map_y = (int)data->ray_y;
	if (data->file->map[*map_y][*map_x] == '1' || is_ray_out_of_bounds(data,
			*map_x, *map_y))
	{
		*hit = 1;
	}
...
void	calculate_arrival_position(t_data *data, double ray_x, double ray_y,
		int arrival_pos[2])
{
