...
void load_texture(t_data *data)
{
	...
	data->ea.img = mlx_xpm_file_to_image(data->mlx.p_mlx, data->file->path->path_ea, &data->ea.width, &data->ea.height);
 	data->ea.addr = mlx_get_data_addr(data->ea.img, &data->ea.bpp, &data->ea.line_lengh, &data->ea.endian);
	if (!data->ea.img)
        printf("Failed to load east texture\n");
	
	data->we.img = mlx_xpm_file_to_image(data->mlx.p_mlx, data->file->path->path_we, &data->we.width, &data->we.height);
 	data->we.addr = mlx_get_data_addr(data->we.img, &data->we.bpp, &data->we.line_lengh, &data->we.endian);
	if (!data->we.img)
